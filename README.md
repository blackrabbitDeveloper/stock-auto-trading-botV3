...
- **손절:** ATR 기반 SL 예약주문 자동 등록
- **트레일링:** 매일 15:40 peak 업데이트, SL 상향 조정
...
| 08:59 | 주문 제출 | 매도(자금확보) → 매수 시장가 주문 |
| 09:05 | 체결 확인 | 체결가 확인 + SL 예약주문 등록 |
| 15:40 | 시그널 생성 | 일봉 수집 → exit 체크 → 신규 매수 시그널 |
...
- **Discord:** 모든 주요 이벤트 알림 (시그널, 주문, 체결, 에러)
- **대시보드:** 실시간 포지션/매매 현황 (30초 자동새로고침)
- **헬스체크:** `/health` 엔드포인트 (Railway 자동 모니터링)

## 안전장치

- 전략당 최대 포지션 수 제한
- 같은 종목 중복 매수 방지 (전 전략 across)
- 주문 실패 시 재시도 없음 → Discord 알림 후 수동 개입
- SL 예약주문으로 장중 급락 방어
